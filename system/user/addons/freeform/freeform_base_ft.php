...
use Solspace\Addons\Freeform\Library\Utils;
...
class Freeform_base_ft
{
		...
		$settings = array(
			$this->field_name => array(
				...
				'settings' => array(
					...
					'desc'		=> '',
					'fields'	=> array(
						$prefix . $data_key => array(
							'label'		=> $lang,
							'desc'		=> '',
							'type'		=> 'yes_no',
							'value'		=> isset($data[$data_key]) ? $data[$data_key] : 'n',
							'required'	=> true,
							'validate'	=> '',
						)
					)
										...
										$desc = 'multi_list_items_desc' )
	{
		...
		$field_list_items = isset($data[$prefix_ . 'field_list_items']) ?
							$data[$prefix_ . 'field_list_items'] :
							'';
		...
		$vars = array(
			'prefix' 				=> $prefix,
			'list_setting'			=> isset($data[$prefix_ . 'list_type']) ?
										$data[$prefix_ . 'list_type'] : '',
	 ...
	 *
	 * @access	public
	 * @return	object		returns an instance of the freeform object
	 */
	...
	protected function fob()
	{
		if (is_null($this->fob))
		{
			if (!class_exists('Freeform')) {
				require_once realpath(dirname(__FILE__) . '/mod.freeform.php');
			}
	...
	public function field_method_link($vars = array(), $real_amp = false, $render = true)
	{
		$base_vars	= array(
			'method'	=> 'field_method'
		);

		if (isset($this->field_id) AND
			$this->is_positive_intlike($this->field_id))
		{
			$base_vars['field_id'] = $this->field_id;
		}

		if (isset($this->form_id) AND
			$this->is_positive_intlike($this->form_id))
		{
		...
		if ( ! empty($vars))
		{
			$base_vars = array_merge($base_vars, $vars);
		}

		$out = $this->fob()->mcp_link($base_vars, $render);
	...
	public function full_stop ($errors = '', $error_type = 'submission')
	{
		...
		return $utils->full_stop($errors, $error_type);
	}
}
