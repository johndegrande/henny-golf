...
class Notifications extends AddonBuilder
{
	// --------------------------------------------------------------------
	...
	public function send_notification($options = array())
	{
		...
		else
		{
			...
			$t_query = $this->model('notification')->get_row(array(
		...
		//	Load parser shim
		// -------------------------------------
		//	This just adds a layer on top of
		//	EE_Template to give us the ability
		//	to load and return strings as parsed
		//	templates.
		// -------------------------------------

		$utilsTemplate   = $this->lib('Utils')->template(true);
		$template_parser =& $utilsTemplate;

		// -------------------------------------
		//	conditionals, date formats, replacements, etc. FUN!
		// -------------------------------------

		$parse_vars = array(array_merge($this->variables, $this->field_outputs));

		$this->subject	= $template_parser->parse_variables(
			$this->subject,
			$parse_vars
		);

		$from_email		= $template_parser->parse_variables(
			$from_email,
			$parse_vars
		);

		$from_name		= $template_parser->parse_variables(
			$from_name,
			$parse_vars
		);

		$reply_to_email	= $template_parser->parse_variables(
			$reply_to_email,
			$parse_vars
		);

		$reply_to_name	= $template_parser->parse_variables(
			$reply_to_name,
			$parse_vars
		);
		...
		//have to run this again since we added variables
		$parse_vars = array(array_merge($this->variables, $this->field_outputs));

		$this->message = $template_parser->parse_variables(
			$this->message,
			$parse_vars
		);
		...
		$this->message = $template_parser->process_string_as_template($this->message);

		// -------------------------------------
		//	restore old parser if any
		// -------------------------------------

		$this->lib('Utils')->restore_template_parser();
		...
		foreach ($recipients as $email_address)
		{
			...
			if ($this->check_yes($this->model('preference')->preference('email_logging')))
			{
				$this->model('email_log')->insert(array(
					"success"		=> ($result ? "y" : "n"),
					...
					"debug_info"	=> ee()->email->print_debugger()
				));
			}
	...
	private function save_spam_interval ($form_id, $entry_id, $email_list)
	{
		...
		$time_check = (
			ee()->localize->now - (
				60 * ( (int) $this->model('preference')->preference('spam_interval') )
			)
		);
	...
	{
		if ($this->check_no($this->model('preference')->preference('enable_spam_prevention')) or
		...
		$t2 = $this->model('form')->table_name($form_id);
		...
		if ( $query->row('email_count') > $this->model('preference')->preference('spam_count') )
		{
			return TRUE;
		}
	 ...
	 *
	 * @access public
	 * @param  mixed 	int/array of ints 	$notification_id to delete
	...
	public function delete_notification($id)
	{
		...
		$this->model('notification')->where('notification_id', $id)->delete();
