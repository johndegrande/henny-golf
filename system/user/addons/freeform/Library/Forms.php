...
//%pro%
											...
											$hash_data = array())
	{
		...
		if ($entry_id != 0 AND $this->model('Data')->is_valid_entry_id($entry_id, $form_id))
		{
...
//%pro%
	...
	public function get_multipage_form_data($form_id, $hash)
	{
		...
		$previous_inputs = $this->model('entry')->get_multipage(
			$form_id,
			$hash
		);
	...
	public function check_keyword_banning()
	{
		...
		$keywords = preg_split(
			"/\r|\n/",
			$this->model('preference')->preference('spam_keywords'),
			-1,
			PREG_SPLIT_NO_EMPTY
		);
