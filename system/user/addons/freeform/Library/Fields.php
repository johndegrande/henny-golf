...
{
	require_once rtrim(dirname(dirname(__FILE__)), '/') . '/freeform_base_ft.php';
}

class Fields extends AddonBuilder
{
	...
	public function & get_field_instance ($options = array())
	{
		...
		if ( ! $use_cache OR ! $cache->is_set())
		{
			...
			$new_instance->show_all_sites	= $this->model('preference')->show_all_sites();
	...
	public function get_default_fieldtypes ($use_cache = TRUE)
	{
		...
		foreach($this->model('Data')->defaults['default_fields'] as $default_field)
		{
	...
	public function uninstall_fieldtype ($name)
	{
		...
		$success = $this->model('fieldtype')->delete(array(
			'fieldtype_name' => $name
		));

		Cacher::clear(__CLASS__, 'get_available_fieldtypes');
	...
	public function install_fieldtype ($name)
	{
		...
		$this->model('fieldtype')->insert(array(
			'fieldtype_name' 	=> $name,
	...
	public function fieldtype_display_settings_output ($name, $settings = array(), $obj = NULL)
	{
		...
		$obj->field_name = strtolower($name);

		return $obj->display_settings($settings);
	}
	...
	{
		//delete existing
		$this->model('fieldtype')->delete(array('default_field' => 'y'));
	...
	public function update_default_freeform_fields ($install_missing = FALSE)
	{
		...
		{
			//delete existing
			$installed =	$this->model('fieldtype')
		...
		return $this->model('fieldtype')->update(
	...
	public function apply_field_method ($options = array())
	{
		...
		{
			$field_input_data = $this->model('Data')->get_entry_data_by_id(
				$entry_id,
				$form_id
			);
		}
	...
	public function delete_field ($field_ids)
	{
		...
		{
			$field_form_data = $this->model('form')->forms_with_field_id($field_id);

			if ( $field_form_data !== FALSE )
			{
				foreach ($field_form_data as $form_id => $form_data)
				{
					$this->lib('Forms')->remove_field_from_form($form_id, $field_id);
				}
			}
		...
		//delete all fields
		$this->model('field')->where_in('field_id', $remove_fields)->delete();

		Cacher::clear(__CLASS__, 'get_field_instance');
