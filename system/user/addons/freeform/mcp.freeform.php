...
class Freeform_mcp extends AddonBuilder
{
		...
		$this->menu = $menu	= array(
			...
			'resources' => array(
				...
				'sub_list' => array(
					...
					'support' => array(
						'link' => 'https://solspace.com/expressionengine/freeform/support/',
	...
	public function edit_form()
	{
		...
		//print_r($inputs['field_order']); exit();
        ...
        $prefs = $this->model('preference');

        $fieldParams = array();
        if ($prefs->preference('default_show_all_site_data') === 'n') {
            $fieldParams['site_id'] = ee()->config->item('site_id');
        }

		$available_fields	= $this->model('field')->get($fieldParams);
		...
		$input_defaults['form_fields']['content'] = $this->setup_select_fields(
			$this->model('field')->key('field_id', 'field_label')->get($fieldParams),
			$this->lib('Utils')->pipe_split($inputs['field_order']),
            $this->lib('Utils')->pipe_split($inputs['field_ids'])
		);
	...
	protected function setup_select_fields($available_fields, $order = array(), $selected = array())
	{
		//---------------------------------------------
		//  Dependencies
		//---------------------------------------------
		...
		$available_fields = array_replace(array_flip($order), $available_fields);
		
		//---------------------------------------------
		//  Field view
		//---------------------------------------------

        $field_view	= ee('View')->make('freeform:publish')->render(
            [
	...
	public function entries($message = '' , $moderate = FALSE, $export = FALSE)
	{
		...
		{
			$current_page				= $this->get_post_or_zero('page') ?: 1;
		...
		foreach ($result_array as $row)
		{
            ...
            $toolbar_items = array();
            if (isset($form_data["entry_url"]) && !empty($form_data["entry_url"])) {
                $entryUrl = $form_data["entry_url"];
                $entryUrl = str_replace("{form_id}", $form_data["form_id"], $entryUrl);
                $entryUrl = str_replace("{form_name}", $form_data["form_name"], $entryUrl);
                $entryUrl = str_replace("{entry_id}", $row['entry_id'], $entryUrl);

                $toolbar_items["view"] = array(
                    "href" => $entryUrl,
                    "title" => lang("entry_url"),
                );
            }

			$toolbar_items['edit'] = array(
		...
		return $this->mcp_view(array(
			...
			'highlight'	=> 'forms',
			'show_message'	=> false,
			'pkg_js'	=> array('standard_cp.min', 'entries_cp.min', 'custom_filters.min', 'edit_form.min'),
	...
	public function edit_entry($message = '')
	{
        ...
        if ($form_data["composer_id"]) {
            ...
            if ($composer !== false) {
                $data = json_decode($composer["composer_data"]);

                $field_order = $data->fields;

                $field_loop_ids = array_merge(
                    $field_order,
                    array_diff($field_loop_ids, $field_order)
                );
            }
        } else if ($form_data['field_order'] !== '') {
	...
	public function edit_field($modal = FALSE)
	{
		...
		foreach ($input_defaults as $short_name => $data)
		{
			...
			if ($data['label'] === 'field_type') {
			    $fieldTypes = array_keys($data['choices']);
			    $fieldTypes = array_combine($fieldTypes, $fieldTypes);

			    $main_section[$short_name]['fields'][$short_name]['group_toggle'] = $fieldTypes;
            }
		}
		//END foreach ($input_defaults as $short_name => $data)
        ...
        foreach ($field_options as $fieldType => $option) {
		    foreach ($option['settings'] as $field) {
		        $sections['settings'][] = array_merge($field, [
                    'group' => $option['group'],
                ]);
            }
        }
	...
	public function templates($message = '')
	{
		...
		$this->cached_vars['form_right_links']	= array(
			array(
				'link' => $this->mcp_link(array('method' => 'edit_template')),
				'title' => lang('new_template'),
			)
		);
		...
		if ( ! $this->model('preference')->show_all_sites())
		{
			$this->model('template')->where('site_id', ee()->config->item('site_id'));
		}

		$total_results = $this->model('template')->count(array(), FALSE);
		...
		if ($total_results > $this->row_limit)
		{
			...
			$this->model('template')->limit(
				$this->row_limit,
				($page - 1) * $this->row_limit
			);
		}

		$this->model('template')->order_by('template_name', 'asc');
			...
			foreach ($query as $row)
			{
				$tableData[] = array(
                    ...
                    array(
                        ...
                        'data'		=> array(
                            'confirm' => lang('template') . ': <b>' . $row['template_id'] . '</b>'
                        )
                    )
				);
			}
        ...
        $this->mcp_modal_confirm(array(
            'form_url'	=> $this->mcp_link(array('method' => 'delete_template')),
            'name'		=> 'templates',
            'kind'		=> lang('templates'),
        ));
		...
		$this->cached_vars += array(
			...
			'cp_page_title'	=> lang('composer_templates'),
			'table_head'	=> lang('composer_templates')
		);
		...
		return $this->mcp_view(array(
			'file'		=> 'list',
			'highlight'	=> 'composer_templates',
			'show_message'	=> false,
			'pkg_js'	=> array(),
			'pkg_css'	=> array('mcp_defaults'),
			'crumbs'	=> array(
				array(lang('composer_templates'))
			)
		));
	}
	//END templates
	...
	public function edit_template()
	{
		...
		if ($update)
		{
			$template_data = $this->model('template')->get_row(array(
				'template_id'	=> $template_id
			));
			...
			if ($template_data !== FALSE)
			{
				...
				$form_info = $this->model('form')->get(array(
					'template_id' => $template_id
				));
				...
				if ($form_info AND ! empty($form_info))
				{
					...
					$lang_template_edit_warning = str_replace(
						'%form_names%',
						implode(', ', $form_names),
		...
		if ( ! $update AND $duplicate_id > 0)
		{
			...
			if ($template_data)
			{
				...
				$duplicated		= TRUE;
				$duplicated_from	= $template_data['template_label'];
			}
		}
		//END if ( ! $update AND $duplicate_id > 0)
		...
		{
			$inputs['param_data'] = json_decode(
				$template_data['param_data'],
				TRUE
			);
		}

		if (! isset($inputs['param_data']) ||
			! is_array($inputs['param_data']))
		{
		...
		$defaultPrefs = $this->model('template')->get_default_prefs();

		//have to build this view here because we do not know
		//inside of default prefs if we have data or not.
		$defaultPrefs['template_params']['content'] = $this->view(
			'value_label_input',
			array('param_data' => $inputs['param_data'])
		);
		...
		foreach ($defaultPrefs as $short_name => $data)
		{
			...
			$data['default']	= str_replace(
				array('%webmaster_name%', '%webmaster_email%'),
				array(
			...
			$main_section[$short_name] = array(
				'wide'		=> isset($data['wide']) ? $data['wide'] : false,
				'hide'		=> (
					isset($data['type']) AND $data['type'] == 'hidden'
				),
				'title'		=> lang($short_name),
				'desc'		=> $desc,
				'fields'	=> $fields
		...
		$new_update	= $update ? 'update_template' : 'new_template';
		...
		$this->cached_vars += array(
			...
			'save_btn_text'			=> $update ?
										'update_template' :
										'create_template',
			'save_btn_text_working'	=> 'saving_template'
		);
		...
		return $this->mcp_view(array(
			...
			'highlight'	=> 'composer_templates',
			'show_message' => false,
			'pkg_js'	=> array('standard_cp.min', 'edit_template_cp.min'),
			'pkg_css'	=> array('mcp_defaults'),
			'crumbs'	=> array(
				array(
					lang('composer_templates'),
					$this->mcp_link(array(
						'method' => 'templates'
					))
				),
	...
	public function permissions($message = '')
	{
		...
		$permissions = $this->model('preference')->preference('permissions');
		...
		$permissions = isset($permissions[$site_id]) ? $permissions[$site_id] : array();
		...
		foreach ($this->menu as $menu_name => $menu_data)
		{
		...
		$global_section = array(
			//'use_global_permissions' => array(
			//	'wide'		=> false,
			//	'title'		=> lang('use_global_permissions'),
			//	'desc'		=> lang('global_permissions_description'),
			//	'fields'	=> array(
			//		'use_global_permissions' => array(
			//			'type'		=> 'yes_no',
			//			'value'		=> $global_permissions ? 'y' : 'n',
			//			'attrs'		=> '',
			//			'required'	=> false
			//		),
			//	)
			//),
			'default_permission_new_group' => array(
				'wide'		=> false,
				'title'		=> lang('default_permissions_new_group'),
				'desc'		=> lang('default_permissions_new_group_desc'),
				'fields'	=> array(
					'default_permission_new_group' => array(
						'type'		=> 'select',
						'choices'	=> array(
							'allow' => lang('allow_all'),
							'deny'	=> lang('deny_all')
						),
						'value'		=> (
							isset($permissions['default_permission_new_group'])
		...
		foreach ($menu_items as $menu_item)
		{
			...
			foreach ($member_groups as $group_id => $group_title)
			{
				...
				{
					$group_yes[] = $menu_item. '_' . $group_id;
				}
			}
			...
			$fields		= array(
				...
				$menu_item . '_member_groups' => array(
					'type'		=> 'checkbox',
		...
		$sections['mcp_tab_permissions']	= $main_section;
		...
		return $this->mcp_view(array(
			...
			'show_message'	=> false,
			'pkg_js'	=> array('permissions_cp.min'),
			'pkg_css'	=> array('mcp_defaults'),
			'crumbs'	=> array(
	...
	public function save_field_layout()
	{
		...
		$form_fields    = ee()->input->get_post('form_fields', TRUE);
		$shown_fields	= isset($form_fields['data']) ? $form_fields['data'] : false;
		$form_id		= $this->get_post_or_zero('form_id');
		$form_data		= $this->model('form')->get_info($form_id);
		...
		foreach ($shown_fields['values'] as $field_name)
		{
			...
			else if (array_key_exists($field_id, $form_data['fields']))
			{
				$data[] = $field_id;
	...
	public function save_form()
	{
        ...
        $postedFieldData = ee()->input->get_post('form_fields', true);
        $field_ids       = (!empty($postedFieldData['data']['values'])) ? $postedFieldData['data']['values'] : [];

        $sorted_field_ids = (!empty($postedFieldData['data']['order'])) ? $postedFieldData['data']['order'] : [];
		...
		$data = array(
			...
			'field_order'				=> implode('|', $sorted_field_ids),
            ...
            'entry_url'                 => ee()->input->get_post("entry_url") ?: null,
		);
	...
	public function save_composer_data()
	{
		...
		{
			$this->lib('Utils')->full_stop(lang('invalid_composer_data'));
		}
		...
		$composer_data_array = json_decode($composer_data, TRUE);
		...
		$composer_data		= json_encode($composer_data_array);
		...
		if ($preview == 'n' AND $form_data['composer_id'] != 0 AND
			$this->model('composer')->count(array(
				'composer_id' => $form_data['composer_id']
			)) != 0)
		{
			$composer_id = $form_data['composer_id'];

			$this->model('composer')->update(
				array(
		...
		{
			$composer_id = $this->model('composer')->insert(array(
				'composer_data'	=> $composer_data,
				'preview'		=> $preview
			));

			$this->model('composer')->clean();
		}
		...
		if ($preview == 'n' )
		{
			$this->lib('Forms')->add_field_to_form($form_id, $composer_fields);
		}
	...
	public function delete_confirm_template($template_id = 0)
	{
		...
		if ( ! is_array($template_id) AND
			 ! $this->is_positive_intlike($template_id))
		{
			$this->lib('Utils')->full_stop(lang('invalid_template_id'));
		}
	...
	public function composer_field_data($field_id = 0, $field_data = NULL, $return = FALSE)
	{
		...
		if ( ! $field_data)
		{
			$field_data = $this->model('field')->get_row($field_id, FALSE);
		}
		...
		$composer_field_data = array(
			...
			'fieldEditUrl'	=> $this->mcp_link(array(
				'method'	=> 'edit_field',
				//this builds a URL, so yes this is intentionally a string
				'modal'	=> 'true',
	...
	public function save_template()
	{
		...
		else
		{
			...
			if ( in_array($template_name, $this->model('Data')->prohibited_names ) )
			{
		...
		{
			$template_id = $this->model('template')->insert(
				array_merge(
					$data,
					array(
						'site_id' => ee()->config->item('site_id')
	...
	public function save_permissions()
	{
		...
		//these can double up due to site id
		...
		$global_permissions		= ee()->input->get_post('use_global_permissions') == 'y';

		$default_permission_ng	= (
			ee()->input->get_post('default_permission_new_group') == 'deny'
		) ? 'deny' : 'allow';

		$permissions = $this->model('preference')->global_preference('permissions');
			...
			if ($allow_type == 'by_group')
			{
				foreach ($member_groups as $group_id)
				{
					$permissions[$site_id][$menu_item]['groups'][$group_id] = (
						ee()->input->get_post($menu_item . '_member_groups') &&
						in_array(
							$menu_item . '_' . $group_id,
							ee()->input->get_post($menu_item . '_member_groups')
						)
					) ? 'y' : 'n';
				}
