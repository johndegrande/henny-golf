...
class File_upload_freeform_ft extends Freeform_base_ft
{
	...
	public function file_uploads_view()
	{
		...
		if ($this->is_positive_intlike(ee()->input->get_post('for_field_id')))
		{
			ee()->freeform_file_upload_model->where(
				'field_id',
				ee()->input->get_post('for_field_id')
			);
		}
		...
		if ($count > 0)
		{
			...
			//END if ($count > $this->paginate_limit)
			...
			if ($files !== FALSE)
			{
				...
				foreach ($files as $k => $file)
				{
					...
					$files[$k]['uprefs_link'] = ee('CP/URL', '/files/directory/' . $uprefs['id'])->compile();
					...
					$tableData[] = array(
						...
						(isset($files[$k]['link']) ?
							array(
								'content' => $files[$k]['filename'],
								'href'	=> $files[$k]['link']
							):
							$files[$k]['filename']
						),
						//forced download link
						(isset($files[$k]['download_link']) ?
							array(
								'content' => lang('download'),
								'href'	=> $files[$k]['download_link']
							):
							lang('cannot_find_file')
						),
						//link to file on front end if possible
						(isset($files[$k]['front_end_link']) ?
							array(
								'content' => $files[$k]['front_end_link'],
								'href'	=> $files[$k]['front_end_link']
							)
							 :
							lang('cannot_find_file')
						),
						...
						//upload prefs link
						array(
							'content' => $files[$k]['uprefs_name'],
							'href'	=> $files[$k]['uprefs_link']
						),
						//checkbox toggle
						array(
							'name'	=> 'file_ids[]',
							'value'	=> $files[$k]['file_id'],
							'data'	=> array(
								'confirm' => '#' . $files[$k]['file_id']
							)
						),
					);
				}
		...
		$table->setNoResultsText('no_files_uploaded');

		$this->fob()->cached_vars['table'] = $table->viewData(
			$this->field_method_link(array(
		...
		$this->fob()->cached_vars += array(
			'base_url'		=> $this->field_method_link(array(
				'field_method'	=> 'delete_file_uploads',
				'form_id'		=> $this->form_id,
				'field_id'		=> $this->field_id
			), false, false),
			'cp_page_title'	=> lang('file_field_uploads'),
			'table_head'	=> lang('file_field_uploads')
		);



		$this->fob()->cached_vars['footer'] = array(
	...
	public function view_file($download = FALSE)
	{
		...
		$mime = ee()->mime_type->ofFile($file_data['server_path'] . $filename);
	 ...
	 * @param	boolean		$update_entry	update the entries with blank info?
	 * @return	null		redirect
	 */

	public function delete_file_uploads ($redirect = TRUE, $file_ids = array(), $update_entry = TRUE)
	{
				...
				if ($d_files !== FALSE)
				{
					...
					if ($update_entry)
					{
						...
						foreach ($field_files as $entry_id => $filenames)
						{
							...
							{
								//skip updating if we are removing this file
	...
	public function pre_process_entries ($entry_ids = array())
	{
		if ( ! empty($entry_ids))
		{
			...
			if ( ! isset($this->cache['db_data'][$this->form_id]))
			{
				$this->cache['db_data'][$this->form_id] = array();
			}

			$cache =& $this->cache['db_data'][$this->form_id];
			...
			$data_query = ee()->freeform_file_upload_model
							->where('form_id', $this->form_id)
							->where_in('entry_id', $entry_ids)
							->get();
			...
			if ($data_query !== FALSE)
			{
				...
				{
					if ( ! isset($cache[$row['entry_id']][$row['field_id']]))
					{
						$cache[$row['entry_id']][$row['field_id']] = array();
					}

					$row['fileurl'] = $upload_prefs['url'] . $row['filename'];

					$cache[$row['entry_id']][$row['field_id']][$row['file_id']] = $row;
				}
			}

			// -------------------------------------
			//	store blanks for not found so
			//	we don't run repeat queries for
			//	items with no data
			// -------------------------------------
	...
	public function replace_tag ($data, $params = array(), $tagdata = FALSE)
	{
		...
		if (isset($cache[$this->entry_id]))
		{
			...
			//again for no reason
			if (isset($cache[$this->entry_id][$this->field_id]))
			{
				$db_data = $cache[$this->entry_id][$this->field_id];
			}
		}
		...
		if (empty($db_data))
		{
			...
			$cache[$this->entry_id][$this->field_id] = $db_data;
		}
			...
			in_array(strtolower($params['total_uploads']), array('yes', 'y', 'true', 'on')))
		{
			if ( ! isset($this->cache['total_counts'][$this->form_id][$this->entry_id]))
			{
				...
				if (isset($cache[$this->entry_id]))
				{
					foreach ($cache[$this->entry_id] as $key => $value)
					{
						$total += count($cache[$this->entry_id][$key]);
					}
				}
				else
				{
	...
	public function display_field ($data = '', $params = array(), $attr = array())
	{
		...
		if ( ! empty( $this->entry_id ) )
		{
			...
			$tagdata	.= '<a class="ff_uploaded_files" href="{freeform:fileurl}" target="_blank">{freeform:filename}</a>';

			if (REQ == 'CP')
			{
				$tagdata .= '&nbsp;&nbsp;<label>';
				$tagdata .= form_checkbox(array(
					'name'		=> $this->field_name . '_delete_file[{freeform:file_id}]',
					'value'		=> '{freeform:file_id}',
					'checked'	=> false
				));
				$tagdata .=  '&nbsp;&nbsp;' . lang('delete') . '</label>';
			}
	...
	public function save ($data)
	{
		...
		if (isset($this->cache[$this->field_name]['upload_array'])) {
			$uploadsArray = $this->cache[$this->field_name]['upload_array'];

			if (!empty($uploadsArray)) {
		...
		if (isset($_POST[$this->field_name . '_delete_file']))
		{
			$deletes = $_POST[$this->field_name . '_delete_file'];
			...
			else if ($this->is_positive_intlike($deletes))
			{
				$delete_ids[] = $deletes;
			}
			...
			if ( ! empty($delete_ids))
			{
				$this->delete_file_uploads(false, $delete_ids);
			}
		}
	...
	public function post_save ($data)
	{
		...
		//END isset($this->cache[$this->field_name]['upload_array'])
	}
	//END post_save
	...
	public function display_settings ($data = array())
	{
		...
		else
		{
			$settings[$ful] = array(
				...
				'fields'	=> array(
					$ful	=> array(
						'type'		=> 'html',
						'content'	=> lang('file_upload_pref_missing_error')
					)
				)
			);
		}
		...
		$settings['allowed_file_types'] = array(
			...
			'attrs'		=> array('id' => 'allowed_file_types'),
			'fields'	=> array(
		...
		$overwrite_on_edit = isset($data['overwrite_on_edit']) ?
								$data['overwrite_on_edit'] :
								$this->default_settings['overwrite_on_edit'];

		$settings['overwrite_on_edit'] = array(
			'title'		=> 'overwrite_on_edit',
			'desc'		=> 'overwrite_on_edit_desc',
			'attrs'		=> array('id' => 'overwrite_on_edit'),
			'fields'	=> array(
				'overwrite_on_edit'	=> array(
					'type'		=> 'yes_no',
					'value'	=> $overwrite_on_edit,
					'required'	=> true
				)
			)
		);
		...
		$settings['disable_xss_clean'] = array(
			'title'		=> 'disable_xss_clean',
			'desc'		=> 'disable_xss_clean_desc',
			'attrs'		=> array('id' => 'disable_xss_clean'),
			'fields'	=> array(
				'disable_xss_clean'	=> array(
					'type'		=> 'yes_no',
					'value'	=> $disable_xss_clean,
					'required'	=> true
				)
			)
		);
	...
	public function validate_settings ($data = array(), $return_settings = FALSE)
	{
		...
		{
			$errors[] = lang('invalid_filetype_filter');
		}
