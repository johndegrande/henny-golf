...
;(function(global, $){
	...
	function setFieldDragging ($buttonObj)
	{
		...
		$buttonObj.draggable({
			...
			'helper'			: function() {
				...
				var $item = $(fieldDrag({'fieldLabel': $buttonObj.text()}));

				//field tags from field dragging
				if ($buttonObj.is('.field_tag'))
				{
					$item.addClass('field_tag');
					$item.attr('data-field-name', $buttonObj.attr('data-field-name'));
				}
				//insert element dragging
				else if ($buttonObj.is('.insert_element'))
				{
					$item.addClass('insert_element');
					$item.attr('data-element-id', $buttonObj.attr('data-element-id'));
				}
	...
	function updateRowEvents ($element, type)
	{
		...
		//	(This is where fields and non_field_fields are dropped)
		// -------------------------------------
		...
		$composerRows.find('tr:not(.no_sort) td:not(.row_control_holder)').sortable({
			...
			'stop'					: function (event, ui)
			{
				...
				if (ui.item.is('.field_tag'))
				{
					...
					var $original = $('div.field_tag[data-field-name="' + fieldName + '"]:not(.field_dragger)');

					//no dupes
					if ($original.hasClass('disabled'))
					{
						$item.remove();
					}
					else
	...
	{
		var $fieldEditForm	= $('#edit_field form:first');
	...
	function setDynrecEditEvents ($elementWrapper)
	{
		...
		$dynrec.on('click', '.freeform_delete_button', function(){
			$(this).parent().remove();
		});
	...
	$(function(){
		...
		//stupid IE10+ removed conditional comments
		//but guess what? Css rendering is _still_
		//different than every other browser.
		...
		$elMcpNav					= $context.closest('.col-group').find('.w-4');
		$mcpNav						= $elMcpNav.find('.box.sidebar');

		// -------------------------------------
		//	template html
		// -------------------------------------

		toggleNavTemplate			= $('#nav_toggle_template').html();
		...
		// -------------------------------------
		//	Move sidebar as soon as possible.
		// -------------------------------------

		$elMcpNav.append($composerSideBar);

		// A fix which allows sticky controls to work properly
		$elMcpNav.parents(".col-group").css('position', 'relative');

		$mcpNav.addClass('mcp-side-nav-dropdown');

		$navTemplate = $(toggleNavTemplate);

		$mcpNav.append($navTemplate);

		$navTemplate.click(function(e){
			$mcpNav.toggleClass('shown');
		});
		...
		$composer.on('click', '.element_wrapper .paragraph_edit.finished', function(){
			var $that		= $(this);
		...
		$composer.on('click', '.element_wrapper .submit_edit.finished', function(){
			var $that		= $(this);
			var $wrapper	= $that.closest('.element_wrapper');
			var $editor		= $wrapper.find('.editor');
		...
		$composer.on('click', '.element_wrapper .submit_previous_edit.finished', function(){
			var $that		= $(this);
			var $wrapper	= $that.closest('.element_wrapper');
			var $editor		= $wrapper.find('.editor');
		...
		$composer.on('click', '.element_wrapper .userrec_edit.finished', function(){
			var $that		= $(this);
			var $wrapper	= $that.closest('.element_wrapper');
			var $editor		= $wrapper.find('.editor');
		...
		$('#field_list').on('click', '.field_tag', function(e){
			var $that		= $(this),
		...
		$('#field_list').on('click', '.field_tag .freeform_edit_button', function(e){
			var $that		= $(this),
		...
		// -------------------------------------
		//	row sorting
		//	(just the row holders in composer itself)
		// -------------------------------------
		...
		$composerRows.sortable({
			...
			'receive'				: function (event, ui) {
				...
				var $item	= $that.data()['ui-sortable'].currentItem;
				//incoming item
				sortOut = true;
		...
		$composer.on('click', '.row_control .row_add_column', function(e){
			var $that	= $(this),
				$tr		= $that.closest('tr'),
		...
		$composer.on('click', '.row_control .row_remove_column', function(e){
			var $that	= $(this),
				$tr		= $that.closest('tr'),
				$tds	= $tr.find('td:not(.row_control_holder)');
		...
		var showPreview = function()
		{
			...
			var composerUrl = Freeform.url.composerPreview +
			...
			if (composerJSON !== previousComposerJSON)
			{
				$.post(
					Freeform.url.composerSave,
					{
						...
						'XID'			: EE.CSRF_TOKEN || Freeform.XID
					},
					function(data)
					{
		...
		var $elNavChild		= $elMcpNav.children().first();
		var sideBarWidth	= $elNavChild.outerWidth();

		var $uiWrapper = $('#composer_wrapper');
		...
		// 	sideBarWidth		= $elNavChild.outerWidth();
		// 	var wrapperBottom	= uiWrapperTop + uiWrapperHeight;
		// 	var scrollTop		= $(this).scrollTop();
    //
		// 	//lets start with this and resize
		// 	$composerSideBar.css('width', '100%');
    //
		// 	//scrolling down?
		...
		// 		}
    //
		// 		$composerSideBar.css('width', sideBarWidth);
		// 	}
		// 	else
		// 	{
		// 		$composerSideBar.css('width', '100%');
		...
		$('body').on('keyup', '#search_fields', function(e){
			var $that = $(this);
